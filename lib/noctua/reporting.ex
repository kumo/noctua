defmodule Noctua.Reporting do
  ...
  def list_this_months_teacher_stats_for_student(%Noctua.Enroling.Student{} = student) do
    partial_query =
      teacher_lessons_query() |> where([l], l.student_id == ^student.id) |> Lesson.this_month()
  ...
  def list_this_months_student_stats_for_teacher(%Noctua.Teaching.Teacher{} = teacher) do
    partial_query =
      student_lessons_query() |> where([l], l.teacher_id == ^teacher.id) |> Lesson.this_month()
  ...
  def list_students_recent_stats do
    # no date
    partial_query = student_lessons_query()
  ...
  def list_teachers_recent_stats do
    # no date
    partial_query = teacher_lessons_query()
  ...
  defp single_stats(query, partial_query) do
    from q in query,
      left_join: pq in subquery(partial_query |> Lesson.present()),
      on: pq.link_id == q.id,
      left_join: aq in subquery(partial_query |> Lesson.absent()),
      on: aq.link_id == q.id,
  ...
  defp all_stats(query, partial_query) do
    ...
    from q in query,
      left_join: tdq in subquery(present_query |> Lesson.today()),
      on: tdq.link_id == q.id,
      left_join: twq in subquery(present_query |> Lesson.this_week()),
      on: twq.link_id == q.id,
      left_join: tmq in subquery(present_query |> Lesson.this_month()),
      on: tmq.link_id == q.id,
      left_join: lmq in subquery(present_query |> Lesson.last_month()),
      on: lmq.link_id == q.id,
      left_join: tmaq in subquery(absent_query |> Lesson.this_month()),
      on: tmaq.link_id == q.id,
      select_merge: %{
        today_count: tdq.count,
        this_week_count: twq.count,
        this_month_count: tmq.count,
        last_month_count: lmq.count,
        absence_count: tmaq.count
      }
  end
  ...
  defp all_combined_stats(query, partial_query) do
    ...
    from q in query,
      left_join: tdq in subquery(partial_query |> Lesson.today()),
      on: tdq.link_id == q.id,
      left_join: twq in subquery(partial_query |> Lesson.this_week()),
      on: twq.link_id == q.id,
      left_join: tmq in subquery(partial_query |> Lesson.this_month()),
      on: tmq.link_id == q.id,
      left_join: lmq in subquery(partial_query |> Lesson.last_month()),
      on: lmq.link_id == q.id,
      left_join: tmaq in subquery(absent_query |> Lesson.last_month()),
      on: tmaq.link_id == q.id,
      select_merge: %{
        today_count: tdq.count,
        this_week_count: twq.count,
