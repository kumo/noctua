defmodule NoctuaWeb.CoreComponents do
  @moduledoc """
  ...
  def input(%{type: "checkgroup"} = assigns) do
    # Logger.error "input"
    # Logger.error assigns[:name]
    # Logger.error assigns[:value]
    # IO.inspect assigns

    ~H"""
    <div phx-feedback-for={@name} class="text-sm col-span-6">
      <.label for={@id}><%= @label %></.label>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(NoctuaWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(NoctuaWeb.Gettext, "errors", msg, opts)
    end
  end
